...
def match():
    ...
    if data_mem:
        ...
        print("************ data.xlsx ************")
        print("***********************************\n")
        print("* 중복을 제외하여 총", str(data_period_length), "개의 내역 중")
        print("일치하는 키워드를 찾지 못한 항목이", len(data_mem), "개 있습니다.\n")
        print(left_align("행번호", 11), left_align("키워드", 30), left_align("지출", 11))
        for row in data_mem:
            print(left_align(row[0], 11), left_align(row[1], 30), left_align(row[2], 11))
    ...
    if bank_expt:
        ...
        print("************ bank.xlsx ************")
        print("***********************************\n")
        print("* 총", str(len(bank_xlsx_data)), "개의 내역 중,\n")
        print("일치하는 키워드를 찾지 못한 항목이", len(bank_expt_length), "개 있습니다.\n")
        print(left_align("행번호", 11), left_align("거래일자", 11), left_align("시각", 11),
              left_align("출금액", 11), left_align("입금액", 11), left_align("거래내용", 11))
        for row in bank_expt:
            print(left_align(row[0], 11), left_align(row[1], 11), left_align(row[2], 11),
                  left_align(row[3], 11), left_align(row[4], 11), left_align(row[5], 11))
